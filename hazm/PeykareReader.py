...
def coarse_pos_e(tags):
    """
    Coarse POS tags of Peykare corpus: N: Noun, V: Verb, AJ: Adjective,
    ADV: Adverb, PRO: Pronoun, DET: Determiner, P: Preposition, POSTP:
    Postposition, NUM: Number, CONJ: Conjunction, PUNC: Punctuation,
    RES: Residual, CL: Classifier, INT: Interjection
    ...
    try:
        return list(
            set(tags) & {'N', 'V',
                         ...
                         'NUM', 'CONJ',
                         'PUNC', 'CL',
                         'INT', 'RES'})[0] + ('e' if 'EZ' in tags else '')
    except:
        return 'N'
...
def join_verb_parts(sentence):
    ...
    before_verbs = join_verb_parts.tokenizer.before_verbs
    after_verbs = join_verb_parts.tokenizer.after_verbs
    verbe = join_verb_parts.tokenizer.verbe
...
class PeykareReader():
    """
    Interfaces [Peykare Corpus](
    http://www.rcisp.com/?q=%D9%BE%DB%8C%DA%A9%D8%B1%D9%87%20%D9%85%D8%AA%D9
    %86%DB%8C%20%D8%B2%D8%A8%D8%A7%D9%86%20%D9%81%D8%A7%D8%B1%D8%B3%DB%8C)
    Bijankhan, M., Sheykhzadegan, J., Bahrani, M., & Ghayoomi, M. (2011).
    Lessons from building a Persian written corpus: Peykare. Language
    Resources and Evaluation, 45, 143–164.
        ...
        for root, dirs, files in os.walk(self._root):
            for name in sorted(files):
                with codecs.open(os.path.join(root, name),
                                 encoding='windows-1256') as peykare_file:
    ...
    def doc_to_sents(self, document):
        ...
        for line in document.split('\r\n'):
            ...
            parts = line.split(' ')
            tags, word = parts[3], self._normalizer.normalize(
                '‌'.join(parts[4:]))
