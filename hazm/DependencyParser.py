...
class MaltParser(MaltParser):
    ...
    def __init__(self, tagger, lemmatizer, working_dir='resources',
    ...
    def parse_tagged_sents(self, sentences, verbose=False):
        ...
        try:
            for sentence in sentences:
                for i, (word, tag) in enumerate(sentence, start=1):
                    ...
                    input_file.write(('\t'.join(
                        [str(i), word.replace(' ', '_'),
                         self.lemmatize(word, tag).replace(' ', '_'), tag, tag,
                         '_',
                         '0', 'ROOT', '_', '_', '\n'])).encode('utf8'))
                input_file.write('\n\n'.encode('utf8'))
            input_file.close()
            ...
            cmd = ['java', '-jar', self._malt_bin, '-w', self.working_dir,
                   '-c', self.mco, '-i', input_file.name, '-o',
                   output_file.name, '-m', 'parse']
            if self._execute(cmd, verbose) != 0:
                raise Exception(
                    "MaltParser parsing failed: %s" % (' '.join(cmd)))

            return (DependencyGraph(item) for item in
                    codecs.open(output_file.name,
...
class TurboParser(ParserI):
    """
    interfaces [TurboParser](http://www.ark.cs.cmu.edu/TurboParser/)
    which you must manually install
    """
    ...
    def tagged_parse_sents(self, sentences):
        input_file = tempfile.NamedTemporaryFile(prefix='turbo_input.conll',
                                                 dir='resources', delete=False)
        output_file = tempfile.NamedTemporaryFile(prefix='turbo_output.conll',
                                                  dir='resources',
                                                  delete=False)
        ...
        try:
            ...
            return (
            DependencyGraph(item, cell_extractor=lambda cells: cells[1:8]) for
            item in
            codecs.open(output_file.name, encoding='utf8').read().split('\n\n')
            if item.strip())
